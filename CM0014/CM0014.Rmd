...
predict(my_lm, gapminder_France2) %>%
  head()
```
Or we can predict on a new dataset: -residuals
```{r}
...
chagning the graph to fit 2 degree poly
```{r}
ggplot(gapminder_Zimbabwe, aes(year, lifeExp)) +
  geom_point() +
  geom_smooth(method = 'lm'??? (missed)formula = y ~ poly(I(x - 1952), degree = 2))
```
